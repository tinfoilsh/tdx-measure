...
fn write_boot_variables(output: &mut Vec<u8>, paths: &PathStorage) -> Result<()> {
    ...
    write_boot_order_analysis(output, &paths.boot_order)?;
    
    // Parse and display Boot#### variables
    let boot_vars = [
        ("Boot0000", &paths.boot_0000),
        ("Boot0001", &paths.boot_0001),
        ("Boot0006", &paths.boot_0006),
        ("Boot0007", &paths.boot_0007),
    ];
    
    for (name, path) in boot_vars {
...
fn write_boot_order_analysis(output: &mut Vec<u8>, boot_order_path: &str) -> Result<()> {
    ...
    // Pretty print BootOrder
    match parse_boot_order(&data) {
        Ok(boot_order) => {
